...
let AnnouncementService = class AnnouncementService {
    ...
    async findAll(companyId) {
        return this.cache.getOrSetVersioned(companyId, ['announcements', 'feed', 'limit:20'], async () => {
            ...
            commentCount.forEach((c) => commentCountMap.set(c.announcementId, c.count));
    ...
    async findAllLimitTwo(companyId) {
        return this.cache.getOrSetVersioned(companyId, ['announcements', 'feed', 'limit:2'], async () => {
            const allAnnouncements = await this.db
                .select({
                id: announcements_schema_1.announcements.id,
                title: announcements_schema_1.announcements.title,
    ...
    async getAllCreateElements(companyId) {
        return this.cache.getOrSetVersioned(companyId, ['announcements', 'create-elements'], async () => {
